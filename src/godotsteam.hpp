...
#include <sdk/public/steam/isteamdualsense.h>
...
//		void enableActionEventCallbacks();
		...
		void triggerSimpleHapticEvent(uint64_t input_handle, int haptic_location, uint32 intensity, const String& gain_db, uint32 other_intensity, const String& other_gain_db);
		void triggerVibration(uint64_t input_handle, uint32 left_speed, uint32 right_speed);
		void triggerVibrationExtended(uint64_t input_handle, uint32 left_speed, uint32 right_speed, uint32 left_trigger_speed, uint32 right_trigger_speed);
...
//		Dictionary receivedRelayAuthTicket();	<------ Uses datagram relay structs which were removed from base SDK
		...
		bool setGenericBytes(const String& reference_name, uint32 data);
		...
		String convertPingLocationToString(uint32 location);
		int estimatePingTimeBetweenTwoLocations(uint32 location1, uint32 location2);
		int estimatePingTimeFromLocalHost(uint32 location);
