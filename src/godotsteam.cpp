...
String Steam::getLaunchCommandLine(){
	...
	String command_line;
	command_line = command_line + commands;
	return command_line;
}
...
String Steam::getAppListInstallDir(uint32_t app_id, int name_max){
	...
	if(SteamAppList() != NULL){
		...
		if(buffer_size != 0){
			directory_name = directory_name + directory_buffer;
		}
...
String Steam::retrieveConnectionDetails(uint64_t host_id){
	...
	String details;
	details = details + connection_details;
	return details;
}
...
void Steam::triggerVibration(uint64_t input_handle, uint32 left_speed, uint32 right_speed){
	if(SteamInput() != NULL){
...
void Steam::triggerVibrationExtended(uint64_t input_handle, uint32 left_speed, uint32 right_speed, uint32 left_trigger_speed, uint32 right_trigger_speed){
	if(SteamInput() != NULL){
...
void Steam::triggerSimpleHapticEvent(uint64_t input_handle, int haptic_location, uint32 intensity, const String& gain_db, uint32 other_intensity, const String& other_gain_db){
	if(SteamInput() != NULL){
...
int Steam::addFavoriteGame(uint32 ip, uint32 port, uint32 query_port, uint32 flags, uint32 last_played){
	...
	return SteamMatchmaking()->AddFavoriteGame(current_app_id, ip, (uint16)port, (uint16)query_port, flags, last_played);
}
...
int Steam::pingServer(const String& ip, uint32 port){
	...
	uint32_t ip4 = *((uint32_t *)address.get_ipv4());
	return SteamMatchmakingServers()->PingServer(ip4, (uint16)port, ping_response);
}
...
int Steam::playerDetails(const String& ip, uint32 port){
	...
	uint32_t ip4 = *((uint32_t *)address.get_ipv4());
	return SteamMatchmakingServers()->PlayerDetails(ip4, (uint16)port, player_response);
}
...
int Steam::serverRules(const String& ip, uint32 port){
	...
	uint32_t ip4 = *((uint32_t *)address.get_ipv4());
	return SteamMatchmakingServers()->ServerRules(ip4, (uint16)port, rules_response);
}
...
int Steam::configureConnectionLanes(uint32 connection, int lanes, Array priorities, Array weights) {
	...
	for(int i = 0; i < lanes; i++){
        //remember this don't regret
        auto a = uint32_t(weights[i]);
		lane_weights[i] = i;
	}
...
bool Steam::setGenericBytes(const String& reference_name, uint32 data){
    const uint8 _data = (uint8)data;
	// const void *this_data = &data;
	// return networking_identities[reference_name.utf8().get_data()].SetGenericBytes(this_data, sizeof(data));
	return networking_identities[reference_name.utf8().get_data()].SetGenericBytes(&_data, sizeof(_data));
}
...
void Steam::setIPv6(const String& reference_name, uint32 ipv6, uint32 port){
    const uint8 _ipv6 = (uint8)ipv6;
	// const uint8 *this_ipv6 = &_ipv6;
	ip_addresses[reference_name.utf8().get_data()].SetIPv6(&_ipv6, (uint16)port);
}
...
int Steam::estimatePingTimeBetweenTwoLocations(uint32 location1, uint32 location2){
	if(SteamNetworkingUtils() == NULL){
		return 0;
	}
	...
	ping_location1.m_data[0] = {(uint8)location1};
	ping_location2.m_data[0] = {(uint8)location2};
...
int Steam::estimatePingTimeFromLocalHost(uint32 location){
	if(SteamNetworkingUtils() == NULL){
		return 0;
	}
	// Add this location to ping struct
	SteamNetworkPingLocation_t ping_location;
	ping_location.m_data[0] = {(unsigned char)location};
...
String Steam::convertPingLocationToString(uint32 location){
	String location_string = "";
	if(SteamNetworkingUtils() != NULL){
		char buffer;
		// Add this location to ping struct
		SteamNetworkPingLocation_t ping_location;
		ping_location.m_data[0] = {(uint8)location};
		SteamNetworkingUtils()->ConvertPingLocationToString(ping_location, &buffer, k_cchMaxSteamNetworkingPingLocationString);
		location_string = location_string + buffer;
	}
